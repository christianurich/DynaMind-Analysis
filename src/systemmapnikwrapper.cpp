...
SystemMapnikWrapper::SystemMapnikWrapper(mapnik::parameters const & params, bool bind,  DM::System * sys , int EPSG, int EPGSTo)
	: datasource(params),
	  sys(sys),
...
void SystemMapnikWrapper::init(mapnik::parameters const& params)
{
	...
	oTargetSRS->importFromEPSG(this->EPGSTo);
...
mapnik::featureset_ptr SystemMapnikWrapper::features(mapnik::query const& q) const
{
	...
	if (extent_.intersects(q.get_bbox()))
	{
		...
		return boost::make_shared<SystemMapnikFeatureset>(q.get_bbox(),desc_.get_encoding(), sys, this->view, this->EPSG, this->EPGSTo);
	}
